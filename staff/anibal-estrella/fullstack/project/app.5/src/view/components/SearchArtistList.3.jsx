...
import { keyPressUtils, replaceRoviLinks } from '../../logic/utilities'
import { searchArtistTivoApi } from '../../logic'
...
export default function SearchArtist({ handleAddArtist, setCurrentArtist }) {
    ...
    const handleRetrieveArtistsList = async () => {
        try {
            freeze();
            const artistList = await searchArtistTivoApi(artistName);
            setSearchArtist(artistList.hits);
            setError(null);
        } catch (error) {
    ...
    const handleRetrieveArtistDetails = async (artistId) => {
        console.log(searchArtistList[0].musicBio.biography.text);

        try {
            freeze();
            const details = await searchArtistList.find(hit => hit.id === artistId);

            if (details.musicBio.biography[0].text) {
                const replacedText = await artistBioRoviLinkReplacer(details.musicBio.biography[0].text);
                details.musicBio.biography[0].text = replacedText;
            }
    ...
    const artistBioRoviLinkReplacer = async (text) => {
        try {
            const result = await replaceRoviLinks(text);
            return result;
        } catch (error) {
            console.error('Error replacing Rovi links:', error);
                                ...
                                {expandedArtists[item.id] && searchArtists && searchArtists[item.id] && (
                                    <div className='pl-3 '>
                                        <div className='bg-gray-100 p-4 mt-4 '>
                                            {searchArtists[item.id].images[0].url && (
                                                <div className='mb-4 '>
                                                    <img className='w-full object-cover aspect-square grayscale rounded-lg border-4 border-gray-400' src={searchArtists[item.id].images[0].url} alt={searchArtists[item.id].name} />
                                                </div>
                                            )}
                                            <h2 className='font-light text-5xl text-gray-400'>{searchArtists[item.id].name}</h2>
                                            {searchArtists[item.id].musicBio.biography[0].text && (
                                                <div>
                                                    <p className='text-gray-400' dangerouslySetInnerHTML={{ __html: searchArtists[item.id].musicBio.biography[0].text }} />
